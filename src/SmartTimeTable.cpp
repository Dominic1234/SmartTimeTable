...
{
 char name[40];
 int mon[9], tue[9], wed[9], thu[9], fri[9];//teacher days       
 char subject[20];
 int mond[9], tues[9], wedn[9], thur[9], frid[9];//teacher class
};

struct day
{
 int mon[9], tue[9], wed[9], thur[9], fri[9];//our class tt
...
{  int pono;
	...
	if(x.mon[pono-1]==0||x.tue[pono-1]==0||x.wed[pono-1]==0||x.thur[pono-1]==0||x.fri[pono-1]==0)
	{
			  if(strcmpi(yun,"Mo")==0)
			  ...
			  if(strcmpi(yun,"Mo")==0)
				 {
						 ...
						 sample[count].mon[pono-1]=y.mon[pono-1];
...
void inputweird(day &x,char ch[3], int suanswer)
{
	if(temp>count)
	 count=temp;
	 count++;
